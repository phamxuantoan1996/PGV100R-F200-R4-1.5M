...
bool pgv100r::connectToPVG()
{
    //     // 1. Open serial port
        
    serial_port = open(port_serial.c_str(), O_RDWR | O_NOCTTY);
    if (serial_port < 0) {
        perror("Error opening serial port");
        return false;
    }
...
bool pgv100r::readDataMatrix()
{
    ...
    if (num_of_byte > 0) 
    {
        if (calculateCRC(buffer) == buffer[20] && num_of_byte >= 21)
        {
            /* code */
            datamatrix = decodeResponse(buffer);
        }
        else
        {
            datamatrix = 0;
...
uint8_t pgv100r::calculateCRC(uint8_t *buffer)
{
    uint8_t crc = 0;
    for(uint8_t i = 0; i < 20; i++)
    {
...
uint64_t pgv100r::decodeResponse(uint8_t *buffer)
{
    uint64_t response = 0;
    //byte8 - byte9 , byte 12 - 17
    response = (buffer[8] << 49) | (buffer[9] << 42) | (buffer[12] << 35) | (buffer[13] << 28) 
